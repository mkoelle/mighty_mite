...
class Servo:
    # these defaults work for the standard TowerPro MG995
    __servo_pwm_freq = 50
    __min_u10_duty = 26 - 0  # offset for correction
    __max_u10_duty = 123 - 10  # offset for correction
    min_angle = 0
    max_angle = 120
    current_angle = 0.001
    ...
    def update_offset(self, min_u10_duty, max_u10_duty):
    ...
    def move(self, angle):
        # round to 2 decimal places, so we have a chance
        # of reducing unwanted servo adjustments
        angle = round(angle, 2)
        # do we need to move?
