...
async def write(request, data):
    await request.write(data.encode("ISO-8859-1") if type(data) is str else data)
...
async def send_file(request, filename, segment=64, binary=False):
    ...
    try:
        ...
        slen = str(stat[6])
        request.headers["Content-Length"] = slen
        if filename.endswith('.' + 'js'):
            request.headers['Content-Type'] = 'text/javascript'
        with open(filename) as f:
            await send_headers(request)
            gc.collect()
            buf = bytearray(min(stat[6], 128))
            while True:
                size = f.readinto(buf)
                if size == 0:
                    break
                await request.write(buf, sz=size)
    except OSError as e:
            ...
            if isinstance(handler, str):
                ...
                try:
                    with open(filename, "r") as f:
                        for line in f:
                            await write(request, line.format(**context))
                except OSError as e:
                    if e.args[0] != errno.ENOENT:
                        raise
                    raise HttpError(request, 404, "File Not Found")
